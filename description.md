...
ソースコードと`README`を掲載したGitHubリポジトリのリンクと、成果物を提出してください。
...
このまま実装を進めて、課題として示されたアプリケーションの仕様を満たしたものを実装してみましょう。以降はご自身で実装を進めていただくため、実装のヒントを掲載します。  
尚、課題に記載していなかった詳細な仕様については自身で検討しながら進めていきますが、以下に示すのはあくまでも一例ですので、従う必要はありません。
...
ここでは、Denoを用いてしりとりアプリを実装する手順を紹介しました。ここまでで実装できたWebアプリケーションをベースにしてカスタマイズすることで、課題を達成できると思います。  
